...
def create_customer_db(query):
    try:
        ...
        if db_connection:
            db_connection.close()
            print("DB connection is closed")
# creates customer in the database with the user-input customer name and email,
# this also returns customer id to be used elsewhere.
...
def db_call_with_values_without_return(query, values):
    try:
        ...
        if db_connection:
            db_connection.close()
            print("DB connection is closed")
# this function is a reusable db call that takes a sql query with values (user input)
# and commits the values into database
